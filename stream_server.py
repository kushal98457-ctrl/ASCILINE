...
# ── BROWSER FILE UPLOAD (pick-from-PC) ──────────────────────────────────────
# Lets the frontend upload a local video instead of only playing files that
# are already sitting in ./videos on the server. Saved under uploads/ with a
# random filename (never the client-supplied name, to avoid path traversal /
# overwrite tricks), then appended to the live playlist queue in pixel_mode
# so the GPU client's ?cols=/?fps= overrides apply to it.
...
async def upload_video(file: UploadFile = File(...)):
    ...
    queue.append({"video": dest_path, "mode": 2, "pixel": True, "vol": 1.0})
...
def _scrub_video_path(v: Optional[int]) -> str:
    queue = getattr(app.state, "queue", [])
    idx   = getattr(app.state, "current_index", 0)
    if v is not None and 0 <= v < len(queue):
        idx = v
...
@app.get("/scrub")
async def scrub_meta(v: Optional[int] = None):
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if not queue:
        # Upload-ready mode: poll until a video is uploaded via /upload
        await websocket.send_text("UPLOAD_READY")
        try:
            while not queue:
                await asyncio.sleep(1.0)
                queue = getattr(app.state, "queue", [])
            # Queue now has at least one item — fall through to playback
        except (WebSocketDisconnect, ConnectionClosed):
            return

    # Normally starts at the beginning of the queue. A GPU client that just
    # uploaded a file (see /upload above) passes ?start_index=<queued_index>
    # so this connection jumps straight to its video instead of playing
    # through everything queued before it. Invalid/out-of-range values fall
    # back to 0, same as before this param existed.
    queue_index = 0  # local index; advances through the queue
    start_index_param = websocket.query_params.get("start_index")
    if start_index_param is not None:
        try:
            candidate = int(start_index_param)
            if 0 <= candidate < len(queue):
    ...
    try:
        while True:
            ...
            # ── GPU CLIENT OVERRIDES (opt-in, per-connection, pixel-mode only) ──
            # ASCILINE V2's GPU renderer consumes the existing pixel_mode raw-BGR
            # stream directly (see frontend-v2/src/network/websocket.ts) instead
            # of the ASCII/text encodings. It needs full source resolution and up
            # to 60 FPS, which the shared `entry` config (from playlist/CLI) isn't
            # aware of. These query params only affect this connection — `entry`
            # itself is never mutated, so other clients watching the same queue
            # entry are unaffected. Values are clamped server-side regardless of
            # what's requested; ASCII/text render modes never read these params.
            if pixel_mode:
                gpu_cols_param = websocket.query_params.get("cols")
                if gpu_cols_param is not None:
                    try:
                        cols = max(16, min(1920, int(gpu_cols_param)))
                    except ValueError:
                        pass
            ...
            MAX_FPS      = 30
            if pixel_mode:
                gpu_fps_param = websocket.query_params.get("fps")
                if gpu_fps_param is not None:
                    try:
                        MAX_FPS = max(1, min(60, int(gpu_fps_param)))
                    except ValueError:
                        MAX_FPS = 30
...
if __name__ == "__main__":
    ...
    # Build the queue — may be empty if no video provided and file not found
    try:
        queue = build_queue(args)
    except Exception as e:
        queue = []
        print(f"[WARN] Could not build queue: {e}")

    if not queue:
        # If user passed an explicit non-default source that's missing, still warn
        explicit_source = (args.playlist or args.folder or
                           (args.video != "video.mp4") or args.webcam)
        if explicit_source:
            print("[ERROR] No videos found. Check your --playlist / --folder / video argument.")
            exit(1)
        else:
            print("[INFO] No video file specified — starting in upload-ready mode.")
            print("[INFO] Drop a video via the browser upload widget to start streaming.")
            queue = []  # Will be populated by /upload endpoint
